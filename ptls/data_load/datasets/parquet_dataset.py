...
class ParquetDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self,
        ...
        if i_filters is not None:
            self.i_filters = IterableChain(*i_filters)
...
class DistributedParquetDataset(ParquetDataset):
    ...
    def _get_my_files(self):
        # there were enumerate(sorted(self.data_files)) before
        my_files = [name for i, name in enumerate(self.data_files) if i % self.real_num_workers == self.real_worker_id]
        return my_files
